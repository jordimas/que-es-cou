...
- Feed data is provided below in this request from raw_feeds_economy_filtered.json
...
- "link_id" must be the exact value of the `link_id` field from the feed item. Never derive, reconstruct, or infer a link_id. If you cannot find the exact link_id in the input, do not include the article.
- Each link_id must appear at most once in the articles array. Never repeat the same article.
- Only include articles whose link_id appears verbatim in the input data. Any article with a link_id not present in the input must be dropped.
- "source" is the publication name (e.g. "Financial Times")
